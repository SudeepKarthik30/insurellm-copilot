...
def evaluate_retrieval(test: TestQuestion, k: int = 10) -> RetrievalEval:
    ...
    print(f"[evaluate_retrieval] question: {test.question}", flush=True)
...
def _to_score(value, default=1.0):
    """
    The judge model sometimes returns a field as a plain number, sometimes
    as a numeric string, and sometimes (inconsistently) nests it inside a
    dict. This pulls a usable float out of whatever shape shows up instead
    of crashing.
    """
    ...
    if isinstance(value, dict):
        for key in ("score", "value", "rating"):
            if key in value:
                return _to_score(value[key], default)
        for v in value.values():
            result = _to_score(v, default=None)
...
def _to_text(value):
    """Flatten a field to plain text regardless of whether the model
    returned a string, a dict, or something else."""
    if isinstance(value, str):
        return value
    if isinstance(value, dict):
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    print(f"[evaluate_answer] question: {test.question}", flush=True)
    generated_answer, retrieved_docs = answer_question(test.question)

    judge_prompt = f"""You are an expert evaluator assessing the quality of answers. Evaluate the generated answer by comparing it to the reference answer. Only give 5/5 scores for perfect answers.
...
Respond in JSON format."""
    ...
    print("[evaluate_answer] calling judge LLM...", flush=True)
    try:
        ...
        # normalize: some models return capitalized keys, and any field
        # (not just feedback) can show up nested in a dict instead of a
        # plain value. Handle every field defensively, not just feedback.
        normalized = {str(k).lower(): v for k, v in parsed.items()}

        answer_eval = AnswerEval(
            feedback=_to_text(normalized.get("feedback", "")),
            accuracy=_to_score(normalized.get("accuracy", 1)),
            completeness=_to_score(normalized.get("completeness", 1)),
            relevance=_to_score(normalized.get("relevance", 1)),
        )
        print("[evaluate_answer] judge success", flush=True)
    except Exception as e:
        print(f"[evaluate_answer] ERROR: {type(e).__name__}: {e}", flush=True)
        raise
...
def evaluate_all_answers():
    """Evaluate all answers to tests. Skips (doesn't crash on) individual
    questions that fail due to transient network/API errors, so one bad
    connection doesn't lose all the results collected so far."""
    tests = load_tests()
    total_tests = len(tests)
    for index, test in enumerate(tests):
        try:
            ...
            print(f"[evaluate_all_answers] SKIPPING question {index} due to: {type(e).__name__}: {e}", flush=True)
            progress = (index + 1) / total_tests
            continue
        progress = (index + 1) / total_tests
        yield test, result, progress
